...
% 0.1 Set Paths
mripath = 'C:\Users\JulianKeil\MEGData CamCan\MRI\';
hspath = 'C:\Users\JulianKeil\MEGData CamCan\MEG\';
gradpath = 'C:\Users\JulianKeil\MEGData CamCan\MEG\FTData\';
outpath = 'C:\Users\JulianKeil\MEGData CamCan\MEG\HeadModel\';

procdat = dir([gradpath, '*.mat']);

%% 1. Read in template source grid
load('C:\Users\JulianKeil\Github\fieldtrip\template\sourcemodel\standard_sourcemodel3d10mm.mat');
...
for p = 1:length(procdat)
    %% 2. Read in individual mri data
        ...
        rawdat = dir([hspath,rawnames(p).name,'\ses-passive\','meg\', '*.fif']);
        hs = ft_read_headshape([hspath,rawnames(p).name,'\ses-passive\','meg\', rawdat.name]);
        
            % 2.1.1 Change cm to mm
            hs.unit = 'mm';
            hs.pos = hs.pos.*10;
            hs.fid.pos = hs.fid.pos.*10;
        
        % 2.2 Read in header
        procdat = dir([gradpath, '*.mat']);
        load([gradpath procdat(p).name]);
        grad = dataw_meg.grad;
    ...
    mris = ft_volumereslice([],mris);
    mris.coordsys = 'neuromag';
    ...
    %% 7. Save
    save([outpath,procdat(p).name],'grad','grid','template_grid','singleshell','lf','-V7.3');
